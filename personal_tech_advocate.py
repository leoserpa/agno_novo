...
agente_advocate = Agent(
    ...
    instructions=[
        ...
        "SUPER CRÍTICO [FORMATO DE RELATÓRIO OBRIGATÓRIO]: Para manter a pontualidade executiva, o seu relatório final DEVE seguir estritamente o template abaixo. Não inverta a ordem, não crie outras numerações e limite-se a 1 único parágrafo denso por seção textual:",
        "TEMPLATE OBRIGATÓRIO:",
        "### 1. Parecer Técnico (Code Review)",
        "(1 parágrafo robusto contendo o veredito do Senior Code Reviewer, avaliando código, testes e arquitetura)",
        "### 2. Visão de Produto e Negócio",
        "(1 parágrafo robusto contendo o veredito da Product Manager, focos em story-telling e valor para a empresa)",
        "### 3. Match com a Vaga e Gaps",
        "(Se houver vaga exigida, escreva a nota 'Match Score: X%'. Abaixo dela, liste rapidamente em bullets os 'Requisitos Atendidos' e os 'Gaps' faltantes no GitHub.)",
        "### 4. Veredito do Headhunter",
        "(1 parágrafo final e direto apontando SIM ou NÃO para a contratação e a sua justificativa executiva)",
